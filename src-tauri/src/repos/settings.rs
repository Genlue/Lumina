...
pub fn get_settings(conn: &Connection, profile_id: &str) -> Settings {
    match conn.query_row(
        "SELECT profile_id, view_mode, sort_by, theme_mode, accent_color,
                ...
                thumb_gen_concurrency, panel_radius
        ...
        |row| Ok(Settings {
            ...
            panel_radius: row.get(40).unwrap_or(12),
        }),
    ) {
        Ok(s) => s,
        Err(_) => {
            conn.execute(
                "INSERT INTO settings (profile_id, view_mode, sort_by, theme_mode, accent_color,
                 ...
                 'custom', '#4A9EFF', '#003D7A',
                 '#4A9EFF', '#003D7A',
                 '#4A9EFF', '#003D7A', 'custom', 'native', '', 10, 12)",
                params![profile_id],
            ).ok();
            Settings {
...
pub fn save_settings(conn: &Connection, profile_id: &str, updates: serde_json::Value) {
    ...
    let panel_radius = updates["panel_radius"]
        .as_i64()
        .unwrap_or(current.panel_radius);
    ...
    conn.execute(
        "UPDATE settings SET view_mode=?1, sort_by=?2, theme_mode=?3, accent_color=?4,
         ...
         thumb_gen_concurrency=?39, panel_radius=?40
         WHERE profile_id=?41",
        params![view_mode, sort_by, theme_mode, accent_color,
                ...
                extract_color_dark, extract_color_light,
                transparent_accent_mode, titlebar_mode, font_family,
                thumb_gen_concurrency, panel_radius, profile_id],
    ).ok();
}
...
mod tests {
    ...
    fn setup() -> Connection {
        ...
        conn.execute_batch(
            "CREATE TABLE settings (
                ...
                panel_radius INTEGER NOT NULL DEFAULT 12
            );"
        ).unwrap();
        conn
    }
    ...
    #[test]
    fn test_transparent_accent_mode() {
        let conn = setup();
        let mut json = serde_json::Map::new();
        json.insert("transparent_accent_mode".to_string(), serde_json::Value::String("system".to_string()));
        save_settings(&conn, "p1", serde_json::Value::Object(json));
        let s = get_settings(&conn, "p1");
    ...
    #[test]
    fn test_font_family_default_and_save() {
        let conn = setup();
        let s = get_settings(&conn, "p1");
        assert_eq!(s.font_family, "", "Default should be empty (system default)");

        // Save a font then verify
        let mut json = serde_json::Map::new();
        json.insert("font_family".to_string(), serde_json::Value::String("Microsoft YaHei UI".to_string()));
        save_settings(&conn, "p1", serde_json::Value::Object(json));
        let s = get_settings(&conn, "p1");
        assert_eq!(s.font_family, "Microsoft YaHei UI");

        // Empty clears back to default and does not touch other fields
        let mut json2 = serde_json::Map::new();
        json2.insert("font_family".to_string(), serde_json::Value::String("".to_string()));
        save_settings(&conn, "p1", serde_json::Value::Object(json2));
        let s = get_settings(&conn, "p1");
        assert_eq!(s.font_family, "");
        assert_eq!(s.view_mode, "grid", "Unrelated fields should be preserved");
    }
}
