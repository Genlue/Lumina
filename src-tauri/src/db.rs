...
fn init_profile_db_schema(conn: &Connection) -> rusqlite::Result<()> {
    ...
    if version < 12 {
        conn.execute_batch(
            "ALTER TABLE settings ADD COLUMN bg_effect_type TEXT NOT NULL DEFAULT 'acrylic';"
        )?;
    ...
    if version < 14 {
        conn.execute_batch(
            "ALTER TABLE settings ADD COLUMN transparent_accent_mode TEXT NOT NULL DEFAULT 'custom';"
        )?;
...
mod tests {
    ...
    fn temp_test_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("lumina_test_{}_{}", name, std::process::id()));
